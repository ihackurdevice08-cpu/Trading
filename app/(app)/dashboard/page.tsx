...
// RiskMiniWidget은 API 호출이 있으므로 lazy load
...
function pct(cur: number, tar: number) {
  return tar ? Math.min(100, (cur / tar) * 100) : 0;
}
function PnLBadge({ v }: { v: number | null }) {
  if (v == null || !Number.isFinite(v)) return <span style={{ opacity: .4 }}>—</span>;
...
function StatCard({ icon, title, value, sub }: {
  icon: string; title: string; value: React.ReactNode; sub?: string;
}) {
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, opacity: .5, letterSpacing: .2 }}>{title}</span>
      </div>
      <div style={{ fontSize: 18, fontWeight: 800, lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 11, opacity: .45, marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (a.ok) setStats(a.stats); else setErr(a.error || "데이터를 불러오지 못했습니다");
  ...
  function toggleRiskWidget() {
    // 대시보드를 끄려 할 때 거래기록도 꺼져있으면 막음
    const next = !rw.dashboard;
    if (!next && !rw.trades) return; // 마지막 하나는 유지
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button
            ...
            title={rw.dashboard ? "리스크 현황 숨기기" : "리스크 현황 표시"}
            style={{
      ...
      {activeGoals.length > 0 && (
        <section>
          <h2 style={{ margin: "0 0 10px", fontSize: 13, fontWeight: 800, opacity: .6, letterSpacing: .3 }}>
            ◎ 진행중 목표
          </h2>
...
function Loading() {
  return <div style={{ padding: 20, opacity: .5, fontSize: 14 }}>불러오는 중…</div>;
}
